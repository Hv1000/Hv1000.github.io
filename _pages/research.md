...
where $\Pi(z)= I_{d_v} - \frac{z \otimes z}{ |z|^2}$ denotes the projection into ${ z }$. The parameter $\gamma$ determines the type of interaction between particles. The most physical relevant case is $d_v =3, \gamma=-3$, which corrsponds to the Coulomb interaction is plasma.
